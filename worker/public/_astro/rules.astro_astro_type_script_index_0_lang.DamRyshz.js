import{a as y}from"./api.Dv2Hb9xf.js";let i=[],o=null,f=!1;document.addEventListener("DOMContentLoaded",async()=>{await E(),S()});async function E(){try{i=await y.rules.list(),B(),i.length>0&&!o&&v(i[0].id)}catch(e){console.error("Failed to load rule sets:",e),r("Failed to load rule sets","error")}}function B(){const e=document.getElementById("rule-set-list");if(e){if(i.length===0){e.innerHTML=`
    ...
    `).join("")}}function p(e){const t=document.getElementById("rule-editor-content"),n=document.getElementById("rule-form");if(!t||!n)return;if(!e){t.style.display="flex",n.style.display="none";return}t.style.display="none",n.style.display="flex";const l=document.getElementById("rule-set-id"),s=document.getElementById("rule-set-name"),c=document.getElementById("rule-set-description"),a=document.getElementById("rule-set-content");l&&(l.value=e.id||""),s&&(s.value=e.name||""),c&&(c.value=e.description||""),a&&(a.value=e.rules_content||"");const d=document.getElementById("ai-enhance-btn"),m=document.getElementById("duplicate-rule-set-btn"),u=document.getElementById("delete-rule-set-btn");d&&m&&u&&(e.id?(d.style.display="flex",m.style.display="flex",u.style.display="flex"):(d.style.display="none",m.style.display="none",u.style.display="none"))}function I(e){const t=document.getElementById("rule-preview-content");if(!t)return;if(!e){t.innerHTML=`
