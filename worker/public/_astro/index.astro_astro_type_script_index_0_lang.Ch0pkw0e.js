import{a as r}from"./api.CmHKHOM5.js";let s=[],d=[],l=null,a=null;document.addEventListener("DOMContentLoaded",async()=>{await v(),await y(),j(),window.innerWidth<769&&u("builder")});async function v(){try{s=await r.requirements.list(),g(),m()}catch(e){console.error("Failed to load requirements:",e),i("Failed to load requirements","error")}}async function y(){try{d=(await r.projects.list()).projects||[],h()}catch(e){console.error("Failed to load projects:",e),i("Failed to load projects","error")}}function u(e){if(window.innerWidth>=769)return;document.querySelectorAll(".panel").forEach(o=>o.classList.remove("active-view")),document.querySelectorAll(".mobile-tab").forEach(o=>o.classList.remove("active"));const t=document.getElementById(`view-${e}`);t&&t.classList.add("active-view");const n=document.querySelector(`[data-view="${e}"]`);n&&n.classList.add("active")}function f(e){l=e;const t=s.find(n=>n.id===e);t&&(document.dispatchEvent(new CustomEvent("requirement-selected",{detail:{requirement:t}})),r.requirements.trackUsage(e).catch(()=>{})),m()}function m(){const e=document.getElementById("requirement-list");if(e){if(s.length===0){e.innerHTML='<div class="empty-state">No requirements yet. Create one!</div>';return}e.innerHTML=s.map(t=>`
      <div class="item-card ${t.id===l?"active":""}" onclick="selectRequirement('${t.id}')">
        <div class="item-card-header">
          <div class="item-name-group">
            <span class="item-name">${c(t.name)}</span>
            ${t.category?`<span class="item-category">${c(t.category)}</span>`:""}
          </div>
        ...
        <p class="item-preview">${c(t.requirements_content)}</p>
      </div>
